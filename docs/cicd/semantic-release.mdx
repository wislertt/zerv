...
- **semantic-release**: _what version is this release?_ It reads conventional commits,
  decides the next version, writes the changelog, tags, and publishes.
- **zerv**: _what version is this exact Git state?_ It computes a version for any commit
...
The reasoning behind this split: [semantic-release stays on main; zerv prereleases every
other branch](https://wisl.dev/blog/semantic-release-zerv/).
...
- The **semantic-release** job publishes and tags releases (`new_release_version`,
  `new_release_published` outputs). The job succeeds on every push to main, even when
  nothing is released: on a non-release commit with no tag on HEAD, `new_release_version`
  is empty and `new_release_published` is `false`. The **zerv-versioning** job runs on
  every valid semantic-release run, via the `if:` gate shown above.
...
output instead of letting zerv detect it. When no release was published and HEAD carries no
tag, the output is empty and zerv falls back to its own tag-based versioning:
...
deploy jobs. [zerv-flow](https://github.com/wislertt/example-zerv-flow) is a working demonstration
repo (trimmed from its [`ci.yml`](https://github.com/wislertt/example-zerv-flow/blob/main/.github/workflows/ci.yml),
...
- [zerv-flow](https://github.com/wislertt/example-zerv-flow) - working demo repo for both halves of this split
