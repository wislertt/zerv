// Test utilities for flow integration tests
...
impl FlowIntegrationTestScenario {
    ...
    pub fn expect_schema_variants(
        self,
        test_cases: Vec<super::scenarios::test_utils::SchemaTestCase>,
    ) -> Self {
...
pub fn test_flow_pipeline_with_schema_test_cases(
    fixture_path: &str,
    schema_test_cases: Vec<super::scenarios::test_utils::SchemaTestCase>,
) {
...
mod tests {
    ...
    #[test]
    fn test_flow_integration_scenario_creation() {
        ...
        assert!(!scenario.test_dir_path().is_empty());

        // Basic flow command should work with version tag
        scenario
            .run_flow_command(&["--output-format", "semver"])
            .assert_success();
    }

    #[test]
    fn test_flow_integration_with_tag() {
        if !should_run_docker_tests() {
            return;
        }
